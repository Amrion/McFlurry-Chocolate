...
TEST(MESSAGES_TABLE_TEST, MESSAGE_delete) {
  MESSAGES msg;
  msg.connect("", "", "");
  msg.setMId(1);
  msg.setDialogId(2);
  ...
  msg.setDate("10.07.2021");
  msg.save();
  msg.delete_by_messageId();
  msg.setMId(1);
  msg.select_by_messageId();
  ASSERT_STREQ(msg.getDate, "");
  msg.close_connection();
}
...
TEST(MARKS_TABLE_TEST, MARKS_select_save) {
    ...
    marks.connect("", "", "");
    marks.setMarkId(1);
    marks.setIdFirstUser(1);
    marks.setIdSecondUser(2);
    marks.setMark(1);
    marks.setDate("20.12.2020");
    marks.save();
    marks.setMarkId(2);
    marks.setIdFirstUser(2);
    marks.setIdSecondUser(1);
    marks.setMark(-1);
...
TEST(MARKS_TABLE_TEST, MARKS_delete) {
    MARKS marks;
    marks.connect("", "", "");
    marks.setMarkId(1);
    marks.setIdFirstUser(1);
...
TEST(PHOTOS_TABLE_TEST, PHOTOS_delete) {
    PHOTOS photos;
    photos.connect("", "", "");
    photos.setPhotoId(1);
    photos.setUserId(1);
