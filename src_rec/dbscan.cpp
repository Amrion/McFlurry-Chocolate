...
void DBScan::fit(std::vector<std::vector<float>> *_X) {
    ...
    for(iter = points.begin(); iter != points.end(); ++iter) {
        if ( iter->cluster_id == UNCLASSIFIED ) {
            if ( expandCluster(*iter, cluster_id) != FAILURE ) {
                cluster_id += 1;
            }
        }
    }
...
int DBScan::expandCluster(point& p, int cluster_id) {
    std::vector<int> clusterSeeds = calculateCluster(p);
    ...
    for (iterSeeds = clusterSeeds.begin(); iterSeeds != clusterSeeds.end(); ++iterSeeds) {
        ...
        if (pointsEqual( points.at(*iterSeeds), p) ) {
            indexCorePoint = idx;
        }
        ++idx;
    }
    ...
    for( size_t i = 0, n = clusterSeeds.size(); i < n; ++i ) {
        std::vector<int> clusterNeighors = calculateCluster(points.at(clusterSeeds[i]));
        if ( clusterNeighors.size() >= min_sample ) {
            std::vector<int>::iterator iterNeighors;
            for ( iterNeighors = clusterNeighors.begin(); iterNeighors != clusterNeighors.end(); ++iterNeighors ) {
                if ( points.at(*iterNeighors).cluster_id == UNCLASSIFIED || points.at(*iterNeighors).cluster_id == NOISE ) {
...
std::vector<int> DBScan::calculateCluster(point& p) {
    int idx = 0;
    std::vector<point>::iterator iter;
    std::vector<int> clusterIndex;
    for( iter = points.begin(); iter != points.end(); ++iter) {
        if ( calculateDistance(p, *iter) <= eps ) {
...
inline float DBScan::calculateDistance(const point& p1, const point& p2) {
    float dist = 0;
    for (size_t i = 0; i < std::min((*X)[p1.i].size(), (*X)[p2.i].size()); ++i) {
        dist += pow( (*X)[p1.i][i] - (*X)[p2.i][i], 2);
    }
    return sqrt(dist);
}

inline float DBScan::calculateDistance(const std::vector<float>& p1, const point& p2) {
    float dist = 0;
...
bool DBScan::pointsEqual(const point& p1, const point& p2) {
...
std::vector<int> DBScan::predict(const std::vector<float>& user_values) {
    ...
    std::vector<point>::iterator iter;
    for(iter = points.begin(); iter != points.end(); ++iter) {
        dists.push_back(calculateDistance(user_values, *iter));
    }
    ...
    auto key_of_max_value = std::max_element(counters.begin(), counters.end(),
