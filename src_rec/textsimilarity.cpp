...
void TextSimilarity::fit(std::list<std::string>& corpus, std::vector<int> _users_id) {
    if (_users_id.size() > 0)
        users_id = _users_id;
    else {
...
float TextSimilarity::predict(const int user1_id, const int user2_id) {
    int u1 = std::distance(users_id.begin(), std::find(users_id.begin(), users_id.end(), user1_id));
    int u2 = std::distance(users_id.begin(), std::find(users_id.begin(), users_id.end(), user2_id));
