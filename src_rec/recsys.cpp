...
void RecSys::fit(const std::vector<std::vector<int>>& V,
                 std::vector<std::vector<float>>* X, std::vector<int> users_id,
                 int _k, float _eps, float _learning_rate, int _nb_epoch,
                 int n_jobs, bool use_reg, float cl_eps, size_t min_sample) {
    recs = fit_predict(V, users_id, _k, _eps, _learning_rate, _nb_epoch, n_jobs,
                       use_reg);
...
std::shared_ptr<std::map<int, std::vector<int>>> RecSys::fit_predict(
    ...
    float _eps, float _learning_rate, int _nb_epoch, int n_jobs, bool use_reg) {
    assert(_k > 0);
    ALS model(_k, _eps = _eps, _learning_rate = _learning_rate,
              _nb_epoch = _nb_epoch, use_reg = use_reg);
    DBScan cluster;
